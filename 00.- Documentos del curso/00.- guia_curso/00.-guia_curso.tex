...
\\
\hline
8  & 05/06 & \textbf{Introduccion al Aprendizaje Automatizado}
  \begin{itemize}
    \item Introduccion a la ciencia de datos y su relacion con inteligencia artificial, aprendizaje automatizado y analisis de datos.
    \item Metodologia de proyectos de ciencia de datos: comprension del negocio, enfoque analitico, requisitos, recopilacion y comprension de datos.
    \item Preparacion de datos: limpieza, valores faltantes, errores, valores anomalos, integracion de fuentes y consolidacion.
    \item Conceptos basicos: dataset, caracteristicas, conjunto de entrenamiento y conjunto de prueba.
    \item Modelado, evaluacion, implementacion y retroalimentacion de modelos.
    \item Tipos de aprendizaje automatizado: supervisado, no supervisado y por refuerzo.
    \item Agrupamiento no supervisado y algoritmo K-Means.
    \item Clasificacion supervisada y algoritmo k-Vecinos Mas Cercanos (kNN).
    \item Discusion de aplicaciones y relacion con problemas de ciberseguridad en entornos cloud.
  \end{itemize}
&
\textbf{Sin tarea}
  \begin{itemize}
    \item No se asigna tarea entregable en esta sesion.
  \end{itemize}
\\
\hline
9  & 07/06 & \textbf{Aprendizaje Automatizado en Google Cloud Platform}
  \begin{itemize}
...
\\
\hline
10 & 08/06 & \textbf{Introduccion al Aprendizaje Automatizado}
  \begin{itemize}
    \item Introduccion a la Ciencia de Datos: concepto, campos relacionados y aplicaciones.
    \item Metodologia fundamental para proyectos de Ciencia de Datos: comprension del negocio, enfoque analitico, requisitos de datos, recopilacion, comprension y preparacion de datos.
    \item Preparacion de datos: limpieza, valores faltantes, errores, valores anomalos, combinacion e integracion de fuentes.
    \item Modelado, evaluacion, implementacion y retroalimentacion de modelos.
    \item Conceptos basicos: dataset, caracteristicas, muestras, conjunto de entrenamiento y conjunto de prueba.
    \item Tipos de aprendizaje automatizado: supervisado, no supervisado y por refuerzo.
    \item Agrupamiento no supervisado: objetivo, medidas de similitud y algoritmo K-Means.
    \item Clasificacion supervisada: motivacion y algoritmo k-Vecinos Mas Cercanos (kNN).
  \end{itemize}
&
\textbf{Sin tarea}
  \begin{itemize}
...
\\
\hline
11 & 12/06 & \textbf{Clase practica: Aprendizaje Automatizado en Google Cloud Platform}
  \begin{itemize}
    \item Creacion del bucket en Cloud Storage y carga del dataset Titanic.
    \item Creacion y configuracion del notebook en Colab Enterprise.
    \item Verificacion del entorno de ejecucion: Python, Pandas y Scikit-Learn.
    \item Carga, exploracion y preparacion del dataset: seleccion de variables, valores faltantes y codificacion de categorias.
    \item Division del dataset en conjuntos de entrenamiento y prueba.
    \item Entrenamiento de un modelo Random Forest con Scikit-Learn.
    \item Evaluacion del modelo: accuracy, matriz de confusion e importancia de variables.
    \item Serializacion del modelo entrenado y almacenamiento del artefacto en Cloud Storage.
    \item Construccion de la API REST con Flask, contenedor Docker y despliegue en Cloud Run.
    \item Prueba del servicio de inferencia mediante \texttt{curl}.
  \end{itemize}
&
\textbf{Tarea}
  \begin{itemize}
    \item Completar el flujo de Machine Learning: preparacion de datos, entrenamiento, evaluacion y prediccion.
    \item Verificar que el modelo entrenado (\texttt{titanic\_model.pkl}) quede almacenado en Cloud Storage.
    \item Construir los archivos \texttt{app.py}, \texttt{requirements.txt} y \texttt{Dockerfile} para la API.
    \item Desplegar el servicio en Cloud Run y validar los endpoints de estado (\texttt{/}) y prediccion (\texttt{/predict}).
    \item Documentar arquitectura, codigo ejecutado, evidencias, errores encontrados, soluciones aplicadas, resultados del modelo y reflexion sobre el uso de ML en cloud.
    \item Entregar reporte en PDF con capturas, comandos relevantes, URL del servicio o evidencia de despliegue, prueba de inferencia y analisis final.
    \item El reporte funciona como evidencia de cierre del modulo de Inteligencia Artificial y Aprendizaje Automatizado en GCP.
  \end{itemize}
\\
\hline
12 & 15/06 & \textbf{Clase practica: Deteccion de APKs maliciosas mediante permisos de Android}
  \begin{itemize}
    \item Planteamiento: clasificacion de malware Android mediante analisis estatico de permisos declarados en \texttt{AndroidManifest.xml}.
    \item Carga, limpieza y representacion de APKs como vectores binarios de permisos.
    \item Analisis exploratorio.
    \item Agrupamiento no supervisado (K-Means) y modelos supervisados (Regresion Logistica, Random Forest, Extra Trees).
    \item Evaluacion.
  \end{itemize}
&
\textbf{Tarea}
  \begin{itemize}
    \item Ejecutar el notebook y contrastar las hipotesis H1--H6 con evidencias del experimento.
    \item Probar la aplicacion web con una APK real e incluir capturas de los resultados.
    \item Entregar reporte en PDF con analisis exploratorio, metricas de los modelos, contraste de hipotesis y reflexion sobre las limitaciones del analisis estatico por permisos.
  \end{itemize}
\\
\hline
13 & 19/06 & \textbf{Taller autónomo I: Análisis exploratorio y modelado no supervisado de APKs}
  \begin{itemize}
    \item Configuracion del entorno de trabajo: Colab Enterprise o entorno local con Python, Pandas y Scikit-Learn.
    \item Carga del dataset de permisos de Android y revision de su estructura.
    \item Limpieza de datos: deteccion de duplicados, manejo de valores faltantes y verificacion de tipos.
    \item Representacion vectorial binaria de APKs a partir de los permisos declarados en \texttt{AndroidManifest.xml}.
    \item Analisis exploratorio: distribucion de permisos por clase, frecuencia de permisos, correlaciones y desequilibrio de clases.
    \item Reduccion de dimensionalidad y visualizacion preliminar de la estructura del dataset.
    \item Aplicacion de K-Means y evaluacion del agrupamiento obtenido.
    \item Documentacion de hallazgos parciales y formulacion de hipotesis para la siguiente sesion.
  \end{itemize}
&
\textbf{Avance esperado al finalizar esta sesion}
  \begin{itemize}
    \item Dataset cargado, limpiado y representado como matriz binaria de permisos.
    \item Analisis exploratorio documentado con visualizaciones y observaciones clave.
    \item Agrupamiento K-Means ejecutado y resultados registrados.
    \item Hipotesis H1--H3 contrastadas con las evidencias obtenidas hasta este punto.
    \item Bitacora tecnica actualizada con comandos, graficas y decisiones tomadas.
  \end{itemize}
\\
\hline
14 & 22/06 & \textbf{Taller autónomo II: Clasificacion supervisada, evaluacion y cierre de la practica}
  \begin{itemize}
    \item Entrenamiento de modelos supervisados: Regresion Logistica, Random Forest y Extra Trees.
    \item Evaluacion comparativa: accuracy, precision, recall, F1-score y matriz de confusion por modelo.
    \item Analisis de importancia de variables: permisos mas relevantes para la clasificacion de malware.
    \item Contraste de hipotesis H4--H6 con las metricas obtenidas en los experimentos.
    \item Prueba de la aplicacion web con una APK real: interpretacion del resultado y captura de evidencias.
    \item Discusion critica sobre las limitaciones del analisis estatico por permisos.
    \item Integracion de todas las evidencias y elaboracion del reporte final.
  \end{itemize}
&
\textbf{Tarea}
  \begin{itemize}
    \item Completar el notebook con los tres modelos supervisados y sus metricas de evaluacion.
    \item Contrastar la totalidad de las hipotesis H1--H6 con evidencias del experimento.
    \item Probar la aplicacion web con una APK real e incluir capturas de los resultados.
    \item Elaborar el reporte final en PDF con: analisis exploratorio, descripcion de modelos, metricas, contraste de hipotesis, evidencias de la prueba web y reflexion sobre las limitaciones del analisis estatico.
  \end{itemize}
\\
\hline
15 & 26/06 & \textbf{Blockchain y Tecnologías Descentralizadas}
  \begin{itemize}
    \item ¿Qué es blockchain?
    \item Estructura de un bloque.
    \item Funciones hash criptográficas.
    \item Transacciones.
    \item Mecanismos de consenso.
    \item Arquitectura de una red blockchain.
    \item Blockchain y ciberseguridad.
    \item Limitaciones y riesgos: escalabilidad, consumo energético, latencia, privacidad e irreversibilidad.
    \item Comparación blockchain vs.\ base de datos convencional.
  \end{itemize}
&
\textbf{Sin tarea}
  \begin{itemize}
    \item No se asigna tarea entregable en esta sesion.
    \item Preparar para la sesion siguiente una VM Linux en GCP con acceso SSH y puertos necesarios abiertos en las reglas de firewall. Se puede reutilizar la infraestructura desplegada en modulos anteriores.
  \end{itemize}
\\
\hline
16 & 29/06 & & \\
...
La bibliografía integra los textos base del curso, los libros disponibles en la carpeta \texttt{06.-Bibliografía}, el repositorio utilizado para el despliegue de ELK Stack y los materiales de apoyo empleados en los temas individuales. No se incluyen materiales ubicados en carpetas denominadas Complementario o Complementarios.
